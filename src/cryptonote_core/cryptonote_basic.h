...
namespace cryptonote
{
  ...
  struct txout_to_key
  {
    ...
    uint64_t amount = 0;  // serialized only in amethyst
    crypto::public_key key;
    crypto::public_key encrypted_secret; // serialized only in amethyst
    uint8_t encrypted_address_type = 0;  // serialized only in amethyst

    BEGIN_SERIALIZE_OBJECT()
      if (ar.bytecoin_amethyst)
        VARINT_FIELD(amount)
      FIELD(key)
      if (ar.bytecoin_amethyst){
        FIELD(encrypted_secret)
        ar.tag("encrypted_address_type");
        ar.serialize_blob(&encrypted_address_type, 1);
  ...
  public:
    ...
    BEGIN_SERIALIZE()
      ...
      FIELD(vin)
      ar.bytecoin_amethyst = version >= 4 && blob_type == BLOB_TYPE_FORKNOTE2;
      FIELD(vout)
  ...
  public:
    ...
    BEGIN_SERIALIZE_OBJECT()
      ...
      else if(version >= 4 && blob_type == BLOB_TYPE_FORKNOTE2) 
      {
        // No signatures in Bytecoin Amethyst coinbase. Signatures in normal transactions have
        // complicated format, we see no reason implementing it here
      }
      else 
      {
